...
// once per page and we never re-upload. See MushafPageImage in the schema.

/**
 * The cached file_ids for the given pages, as a `page -> file_id` map. Pages
 * not yet cached are simply absent from the map. One query for the whole wird.
 */
export async function getCachedPageImageIds(pages: number[]): Promise<Map<number, string>> {
 ...
 */
export function cachePageImageId(page: number, fileId: string) {
